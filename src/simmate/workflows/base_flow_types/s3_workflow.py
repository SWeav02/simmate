...
class S3Workflow(Workflow):
    ...
    def execute(cls, directory: Path, command: str) -> list[tuple[str]]:
        ...
        while len(corrections) <= cls.max_corrections:
            ...
            # BUG-FIX This needs to happen after the final error handler check
            # in case the job was terminated by an error before the monitors
            # could catch it. This happens with the VASP Zbrent() error handler
            # for example. -S. Weaver
